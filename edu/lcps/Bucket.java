...
//2019-01-12 - modified by Fred Morrison to provide getWaterLevel() method
//which will allow students to monitor the current water level in a bucket.
package edu.lcps;
...
public class Bucket {
	...
	public static Image getImage() {
		...
		if (max == 0)
			win = false;
		for (int x = 60; x < 550 - 500 / doneList.length; x += 500 / doneList.length) {
			if (count > max)
				g.setColor(Color.gray);
		...
		for (int index = 0; index < numBuckets; index++) {
			...
			for (int loop = 1; loop <= jugList[index].capacity; loop++) {
				g.drawString("" + loop, start - 20, 300 - (int) (scale * (loop - 0.5)));
			}
			g.setColor(Color.white);
			g.fillRect(start, 300 - scale * jugList[index].capacity, end - start, scale * jugList[index].capacity);
			g.setColor(Color.black);
			g.drawRect(start, 300 - scale * jugList[index].capacity, end - start, scale * jugList[index].capacity);
			g.setColor(Color.blue);
			if (301 - (int) (scale * jugList[index].water) < 300 - scale * jugList[index].capacity) {
				g.fillRect(start + 1, 301 - scale * jugList[index].capacity, end - start - 1,
						scale * jugList[index].capacity - 1);
			} else
				g.fillRect(start + 1, 301 - (int) (scale * jugList[index].water), end - start - 1,
						(int) (scale * jugList[index].water) - 1);
		}
		...
		if (win) {
			if (!shownWin) {
				...
				for (int z = 1; z <= 7; z++) {
					...
					/*
					 * int y = 500; if(z == 1) y = 750; try { Thread.sleep(y); } catch
